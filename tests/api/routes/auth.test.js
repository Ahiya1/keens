    ...
    describe('POST /api/v1/auth/login', () => {
        ...
        beforeAll(async () => {
            ...
            try {
                ...
                testEmails.push(testEmail);
                testUser = await createTestUserInDB(testEmail, testUsername, 'TestPassword123!');
            }
            catch (error) {
    ...
    describe('API Key Management', () => {
        ...
        beforeAll(async () => {
            ...
            try {
                ...
                const adminUser = await createTestUserInDB(adminEmail, adminUsername, 'TestPassword123!', true // isAdmin
                );
    ...
    describe('Admin Authentication', () => {
        ...
        describe('POST /api/v1/auth/admin/verify', () => {
            ...
            test('should require admin privileges', async () => {
                ...
                testEmails.push(regularEmail);
                try {
                    const regularUser = await createTestUserInDB(regularEmail, `regular_${Date.now()}`, 'TestPassword123!', false // not admin
                    );
                    const regularLogin = await request(app)
                        .post('/api/v1/auth/login')
                        .send({
