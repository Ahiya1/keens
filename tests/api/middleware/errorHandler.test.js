 ...
 */
import { errorHandler, notFoundHandler, asyncHandler, createValidationError, createAuthError, createRateLimitError } from '../../../src/api/middleware/errorHandler.js';
import { AuthenticationError, ValidationError, RateLimitError, ConcurrencyError, MFARequiredError } from '../../../src/api/types.js';
describe('Error Handler Middleware', () => {
    ...
    describe('errorHandler', () => {
        ...
        test('should handle Anthropic API errors', () => {
            ...
            errorHandler(anthropicError, req, res, next);
            expect(res.status).toHaveBeenCalledWith(503);
            expect(res.json).toHaveBeenCalledWith({
                success: false,
                error: {
        ...
        test('should log error information correctly', () => {
            ...
            errorHandler(error, req, res, next);
            expect(consoleErrorSpy).toHaveBeenCalledWith('🚨 API Error:', expect.stringContaining('TestError'));
        });
        test('should handle admin user context in error logging', () => {
            ...
            errorHandler(error, req, res, next);
            expect(consoleErrorSpy).toHaveBeenCalledWith('🚨 API Error:', expect.stringContaining('"isAdmin":true'));
        });
    });
