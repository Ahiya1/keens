...
describe('Rate Limiting Middleware', () => {
  ...
  describe('Rate Limit Message Configuration', () => {
    ...
    test('should have different limits for agent execution', () => {
      ...
      const agentExecutionCall = mockRateLimit.mock.calls.find(call => 
        call[0].message?.error?.code === 'AGENT_EXECUTION_LIMIT_EXCEEDED'
      );
    ...
    test('should have auth-specific rate limiting configuration', () => {
      ...
      const authRateLimitCall = mockRateLimit.mock.calls.find(call => 
        call[0].message?.error?.code === 'AUTH_RATE_LIMIT_EXCEEDED'
      );
  ...
  describe('Concurrent Sessions Management', () => {
    beforeEach(() => {
      ...
      const activeSessions = (require('../../../src/api/middleware/rateLimiting.js') as any).activeSessions;
    ...
    test('should allow request within concurrent session limit', () => {
      ...
      concurrencyMiddleware(req as Request, res as Response, next);
  ...
  describe('Different Rate Limit Tiers', () => {
    test('should configure different limits for different endpoints', () => {
      ...
      const agentConfig = mockRateLimit.mock.calls.find(call => 
        call[0].message?.error?.code === 'AGENT_EXECUTION_LIMIT_EXCEEDED'
      );
      if (agentConfig) {
      ...
      const authConfig = mockRateLimit.mock.calls.find(call => 
        call[0].message?.error?.code === 'AUTH_RATE_LIMIT_EXCEEDED'
      );
      if (authConfig) {
