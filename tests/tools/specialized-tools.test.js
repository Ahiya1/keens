...
describe('Specialized Agent Tools', () => {
    describe('Testing Agent Tools', () => {
        ...
        it('should support test execution and reporting', async () => {
            const testRunner = {
                executeTests: async (testFiles, options) => {
                ...
                validateSilentMode: (result) => {
                    // Silent mode should have minimal output but still report results
                    return result.totalTests > 0 && result.duration > 0;
                },
            };
            const result = await testRunner.executeTests(['basic.test.ts', 'core.test.ts', 'validation.test.ts'], { silent: true, coverage: true });
        ...
        it('should support vulnerability scanning', () => {
            const securityScanner = {
                ...
                generateSecurityReport: (scanResults) => {
                    ...
                    const criticalCount = scanResults.reduce((sum, result) => sum + result.vulnerabilities.filter(v => v.severity === 'critical').length, 0);
                    const averageScore = scanResults.reduce((sum, result) => sum + result.score, 0) / scanResults.length;
                    return {
    ...
    describe('Database Agent Tools', () => {
        it('should support schema migration tools', () => {
            const migrationTool = {
                createMigration: (description, upSQL, downSQL) => {
                ...
                planMigrations: (migrations) => {
                    ...
                    return {
                        ...
                        hasDestructiveOperations: sortedMigrations.some(m => m.up.toLowerCase().includes('drop')),
                    };
                },
            };
            // Test migration creation
            const userTableMigration = migrationTool.createMigration('create users table', 'CREATE TABLE users (id UUID PRIMARY KEY, email TEXT UNIQUE NOT NULL);', 'DROP TABLE IF EXISTS users;');
            ...
            const badMigration = migrationTool.createMigration('dangerous migration', 'DROP TABLE users;', // No IF EXISTS
...
//# sourceMappingURL=specialized-tools.test.js.map