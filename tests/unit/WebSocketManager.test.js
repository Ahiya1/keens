...
describe("WebSocketManager", () => {
    ...
    beforeEach(() => {
        ...
        webSocketManager = new WebSocketManager(mockWss, mockKeenDB, mockAuditLogger);
    });
    ...
    describe("connection handling", () => {
        ...
        it("should handle connection with session filters", () => {
            const mockWs = new MockWebSocket();
            const mockReq = createMockRequest("/?token=valid-token&sessions=session1,session2");
            mockWss.emit("connection", mockWs, mockReq);
            const message = mockWs.getLastMessage();
        ...
        it("should reject connection without token", () => {
            ...
            mockWss.emit("connection", mockWs, mockReq);
            expect(closeSpy).toHaveBeenCalledWith(1008, "Authentication token required");
        });
    ...
    describe("broadcasting", () => {
        ...
        beforeEach(() => {
            mockWs = new MockWebSocket();
            const mockReq = createMockRequest("/?token=valid-token&sessions=test-session");
            mockWss.emit("connection", mockWs, mockReq);
    ...
    describe("connection lifecycle", () => {
        it("should handle connection close", () => {
            ...
            expect(logs.some((log) => log.action === "websocket_disconnect")).toBe(true);
        });
    ...
    describe("heartbeat and cleanup", () => {
        ...
        it("should send ping to connections during heartbeat", () => {
            ...
            const timerWebSocketManager = new WebSocketManager(timerMockWss, mockKeenDB, mockAuditLogger);
            const mockWs = new MockWebSocket();
            const mockReq = createMockRequest("/?token=valid-token");
            ...
            const manager = timerWebSocketManager;
