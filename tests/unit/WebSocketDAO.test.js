    ...
    async query(text, params, context) {
        if (text.includes("INSERT INTO websocket_connections")) {
            const connection = {
                ...
                subscribed_events: params?.[7] || [],
                session_filters: params?.[8] || [],
        ...
        if (text.includes("UPDATE websocket_connections") &&
            text.includes("last_ping_at")) {
            const connectionId = params?.[0];
            const connection = this.connections.find((c) => c.connection_id === connectionId && c.connection_status === "active");
            if (connection) {
        ...
        if (text.includes("SELECT * FROM websocket_connections") &&
            text.includes("WHERE session_id = $1") &&
            text.includes("connection_status = 'active'") &&
            text.includes("ORDER BY connected_at DESC")) {
            const sessionId = params?.[0];
            return this.connections.filter((c) => c.session_id === sessionId && c.connection_status === "active");
        }
        ...
        if (text.includes("SELECT * FROM websocket_connections") &&
            ...
            return this.connections.filter((c) => c.user_id === userId && c.connection_status === "active");
        }
        ...
        if (text.includes("SELECT * FROM websocket_connections") &&
            ...
            !text.includes("ANY(subscribed_events)")) {
            return this.connections.filter((c) => c.connection_status === "active");
        }
        if (text.includes("UPDATE websocket_connections") &&
        ...
        if (text.includes("SELECT * FROM websocket_connections WHERE connection_id")) {
            const connectionId = params?.[0];
            const connection = this.connections.find((c) => c.connection_id === connectionId);
        ...
        if (text.includes("COUNT(*) as total_connections")) {
            ...
            const activeConnections = this.connections.filter((c) => c.connection_status === "active").length;
            return [
                {
        ...
        if (text.includes("UPDATE websocket_connections") &&
            ...
            const inactiveConnections = this.connections.filter((c) => c.connection_status === "active" &&
            ...
            return inactiveConnections.map((c) => ({ id: c.id }));
        }
        return [];
    }
}
describe("WebSocketDAO", () => {
    ...
    beforeEach(() => {
        ...
        webSocketDAO = new WebSocketDAO(mockDb);
    });
    ...
    describe("updateConnectionActivity", () => {
        ...
        it("should return false for non-existent connection", async () => {
            const result = await webSocketDAO.updateConnectionActivity("non-existent");
    ...
    describe("getAllActiveConnections", () => {
        it("should return all active connections for admin", async () => {
            ...
            const adminContext = { userId: "admin-id", isAdmin: true };
            const connections = await webSocketDAO.getAllActiveConnections(adminContext);
            expect(connections).toHaveLength(2);
        });
        it("should throw error for non-admin user", async () => {
            const userContext = { userId: "user-id", isAdmin: false };
            await expect(webSocketDAO.getAllActiveConnections(userContext)).rejects.toThrow("Admin privileges required to view all connections");
        });
        it("should throw error for undefined context", async () => {
            await expect(webSocketDAO.getAllActiveConnections()).rejects.toThrow("Admin privileges required to view all connections");
        });
    });
    ...
    describe("updateConnectionSubscriptions", () => {
        it("should update subscriptions for active connection", async () => {
            ...
            const result = await webSocketDAO.updateConnectionSubscriptions("ws-conn-456", newEvents, newFilters);
            expect(result).toBe(true);
        });
        it("should return false for non-existent connection", async () => {
            const result = await webSocketDAO.updateConnectionSubscriptions("non-existent", ["message"], []);
            expect(result).toBe(false);
        });
        it("should return false for closed connection", async () => {
            ...
            await webSocketDAO.closeConnection("ws-conn-456");
            const result = await webSocketDAO.updateConnectionSubscriptions("ws-conn-456", ["message"], []);
            expect(result).toBe(false);
        });
    });
    describe("getConnectionMetrics", () => {
        it("should return metrics for admin", async () => {
            ...
            const adminContext = { userId: "admin-id", isAdmin: true };
            const metrics = await webSocketDAO.getConnectionMetrics(undefined, undefined, adminContext);
        ...
        it("should return metrics with date range for admin", async () => {
            ...
            const adminContext = { userId: "admin-id", isAdmin: true };
            const metrics = await webSocketDAO.getConnectionMetrics(startDate, endDate, adminContext);
            expect(typeof metrics.totalConnections).toBe("number");
            expect(typeof metrics.activeConnections).toBe("number");
        });
        it("should throw error for non-admin user", async () => {
            const userContext = { userId: "user-id", isAdmin: false };
            await expect(webSocketDAO.getConnectionMetrics(undefined, undefined, userContext)).rejects.toThrow("Admin privileges required for connection metrics");
        });
    });
    describe("cleanupInactiveConnections", () => {
        it("should cleanup inactive connections for admin", async () => {
            ...
            const adminContext = { userId: "admin-id", isAdmin: true };
            const cleaned = await webSocketDAO.cleanupInactiveConnections(30, adminContext);
            expect(cleaned).toBeGreaterThanOrEqual(0);
        });
        it("should use default threshold when not specified", async () => {
            const adminContext = { userId: "admin-id", isAdmin: true };
            const cleaned = await webSocketDAO.cleanupInactiveConnections(undefined, adminContext);
            expect(typeof cleaned).toBe("number");
        });
        it("should throw error for non-admin user", async () => {
            const userContext = { userId: "user-id", isAdmin: false };
            await expect(webSocketDAO.cleanupInactiveConnections(30, userContext)).rejects.toThrow("Admin privileges required for connection cleanup");
        });
    });
    ...
    describe("getEventSubscribers", () => {
        ...
        it("should filter by session when provided", async () => {
            ...
            const subscribers = await webSocketDAO.getEventSubscribers("message", "session-123");
            expect(subscribers).toHaveLength(1);
            expect(subscribers[0].connection_id).toBe("ws-conn-1");
        });
        it("should include connections with no session filters", async () => {
            ...
            const subscribers = await webSocketDAO.getEventSubscribers("message", "any-session");
            expect(subscribers).toHaveLength(1);
            expect(subscribers[0].connection_id).toBe("ws-conn-1");
        });
    });
});
//# sourceMappingURL=WebSocketDAO.test.js.map