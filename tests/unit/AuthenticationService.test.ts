...
describe('AuthenticationService', () => {
  ...
  beforeEach(() => {
    ...
    mockAuditLogger = new AuditLogger({} as any) as jest.Mocked<AuditLogger>;
  ...
  describe('login', () => {
    ...
    test('should successfully login regular user with valid credentials', async () => {
      ...
      expect(mockAuditLogger.logSuccessfulLogin).toHaveBeenCalledWith(
        mockUser.id,
        mockClientInfo,
    ...
    test('should successfully login admin user with extended token expiry', async () => {
      ...
      (bcrypt.compare as jest.Mock).mockResolvedValue(true);
      (jwt.sign as jest.Mock).mockReturnValue('mock-admin-jwt-token');
      mockDB.query.mockResolvedValue([]);
    ...
    test('should fail login with invalid email', async () => {
      ...
      await expect(
        authService.login(mockCredentials, mockClientInfo)
      ).rejects.toThrow(AuthenticationError);
      
      expect(mockAuditLogger.logFailedLogin).toHaveBeenCalledWith(
    ...
    test('should skip rate limiting for admin email', async () => {
      ...
      mockUserDAO.getUserByEmailForAuth = jest.fn().mockResolvedValue(adminUser);
      (bcrypt.compare as jest.Mock).mockResolvedValue(true);
      (jwt.sign as jest.Mock).mockReturnValue('admin-jwt-token');
      mockDB.query.mockResolvedValue([]);
  ...
  describe('generateAccessToken', () => {
    ...
    test('should generate JWT with admin scopes for admin user', async () => {
      ...
      const token = await authService.generateAccessToken(adminUser as any);
  ...
  describe('verifyAccessToken', () => {
    ...
    test('should throw error when user no longer exists', async () => {
      ...
      await expect(
        authService.verifyAccessToken('token-for-deleted-user')
      ).rejects.toThrow('Token is invalid - user account not active');
    });
    ...
    test('should throw error when admin privileges revoked', async () => {
      ...
      await expect(
        authService.verifyAccessToken('revoked-admin-token')
      ).rejects.toThrow('Admin privileges have been revoked');
    });
  });

  describe('createAPIKey', () => {
    const mockKeyConfig: APIKeyConfig = {
      name: 'Test API Key',
    ...
    test('should throw error when user not found', async () => {
      ...
      await expect(
        authService.createAPIKey(userId, mockKeyConfig, context)
  ...
  describe('validateAPIKey', () => {
    ...
    test('should throw error for expired API key', async () => {
      ...
      await expect(
        authService.validateAPIKey(keyValue)
  ...
  describe('refreshAccessToken', () => {
    test('should refresh valid token', async () => {
      ...
      (authService as any).refreshTokens.set(refreshToken, {
        userId: mockUser.id,
        createdAt: new Date(),
