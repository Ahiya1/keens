...
describe('CreditGatewayService', () => {
    let creditGatewayService;
    let mockCreditDAO;
    let mockUserDAO;
    let mockDB;
    beforeEach(() => {
        // Create mocked instances
        mockCreditDAO = new CreditDAO({});
    ...
    describe('validateAndReserveCredits', () => {
        ...
        test('should allow admin users with unlimited credits', async () => {
            ...
            const result = await creditGatewayService.validateAndReserveCredits(userId, mockAgentRequest, context);
        ...
        test('should throw InsufficientCreditsError when balance is low', async () => {
            ...
            // Mock transaction history for daily usage
            mockDB.query.mockResolvedValueOnce([{ daily_spent: 0 }]);
            await expect(creditGatewayService.validateAndReserveCredits(userId, mockAgentRequest, context)).rejects.toThrow('Insufficient credits');
        });
        test('should handle high complexity vision requests', async () => {
            ...
            mockCreditDAO.getCreditAccount = jest.fn().mockResolvedValue(mockAccount);
            mockDB.query.mockResolvedValueOnce([{ daily_spent: 0 }]);
            const result = await creditGatewayService.validateAndReserveCredits(userId, complexRequest, context);
    ...
    describe('finalizeCredits', () => {
        test('should handle admin bypass finalization', async () => {
            ...
            const result = await creditGatewayService.finalizeCredits(userId, reservationId, actualClaudeCost, sessionId, context);
            expect(result.is_admin_bypass).toBe(true);
            expect(mockCreditDAO.deductCredits).toHaveBeenCalledWith(expect.objectContaining({
                userId,
    ...
    describe('getBalance', () => {
        ...
        test('should throw error when account not found', async () => {
            ...
            await expect(creditGatewayService.getBalance(userId, context)).rejects.toThrow('Credit account not found');
        });
    });
    describe('addCredits', () => {
        test('should add credits to user account', async () => {
            ...
            const result = await creditGatewayService.addCredits(userId, amount, description, paymentReference, context);
            expect(result).toEqual(mockTransaction);
            expect(mockCreditDAO.addCredits).toHaveBeenCalledWith(expect.objectContaining({
                userId,
                amount,
                description,
                paymentReference,
    ...
    describe('getTransactionHistory', () => {
        test('should return paginated transaction history with summary', async () => {
            ...
            const result = await creditGatewayService.getTransactionHistory(userId, options, context);
    ...
    describe('getAdminAnalytics', () => {
        test('should return admin analytics for authorized admin users', async () => {
            ...
            const result = await creditGatewayService.getAdminAnalytics(adminUserId, timeRange, context);
        ...
        test('should reject non-admin users', async () => {
            ...
            await expect(creditGatewayService.getAdminAnalytics(regularUserId, 'day', context)).rejects.toThrow('Admin privileges required for analytics access');
        });
    });
    describe('vision complexity analysis', () => {
        test('should correctly analyze vision complexity levels', async () => {
            ...
            for (const testCase of testCases) {
                ...
                const result = await creditGatewayService.validateAndReserveCredits(userId, request, context);
                expect(result.estimatedCost).toBeGreaterThan(testCase.expectedMinCost);
            }
        });
    });
});
//# sourceMappingURL=CreditGatewayService.test.js.map