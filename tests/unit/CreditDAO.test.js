...
class MockDatabaseManager {
    accounts = []; // Made public for test access
    transactions = []; // Made public for test access
    async query(text, params, context) {
        ...
        if (text.includes('SELECT is_admin FROM users')) {
            const userId = params?.[0];
            return [{ is_admin: userId === 'admin-user-id' }];
        }
    ...
    async transaction(callback, context) {
        const mockTransaction = {
            query: (text, params) => this.query(text, params, context)
        };
...
describe('CreditDAO', () => {
    ...
    beforeEach(() => {
        ...
        creditDAO = new CreditDAO(mockDb);
    });
    ...
    describe('deductCredits', () => {
        ...
        it('should deduct credits with 5x markup for regular users', async () => {
            ...
            const context = { userId: 'regular-user-id', isAdmin: false };
            const transaction = await creditDAO.deductCredits(request, context);
...
//# sourceMappingURL=CreditDAO.test.js.map