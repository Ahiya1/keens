...
import { AnalyticsDAO, } from "../../src/database/dao/AnalyticsDAO.js";
...
class MockDatabaseManager {
    ...
    async query(text, params, context) {
        ...
        if (text.includes("SELECT * FROM daily_analytics") &&
        ...
        if (text.includes("COUNT(*) as total_sessions") &&
        ...
        if (text.includes("COUNT(*) FILTER (WHERE execution_status = 'running') as active_sessions") &&
        ...
        if (text.includes("SUM(current_balance)") ||
        ...
        if (text.includes("INSERT INTO daily_analytics") ||
...
describe("AnalyticsDAO", () => {
    ...
    beforeEach(() => {
        ...
        analyticsDAO = new AnalyticsDAO(mockDb);
    });
    describe("getDailyAnalytics", () => {
        it("should get analytics for date range", async () => {
            const startDate = new Date("2023-01-01");
            const endDate = new Date("2023-01-31");
            const context = { userId: "user-123", isAdmin: false };
            const analytics = await analyticsDAO.getDailyAnalytics(startDate, endDate, undefined, context);
        ...
        it("should get analytics for specific user", async () => {
            ...
            const analytics = await analyticsDAO.getDailyAnalytics(startDate, endDate, userId, context);
            expect(analytics).toHaveLength(1);
        ...
        it("should allow admin to view system-wide analytics", async () => {
            const startDate = new Date("2023-01-01");
            const endDate = new Date("2023-01-31");
            const adminContext = { userId: "admin-123", isAdmin: true };
            const analytics = await analyticsDAO.getDailyAnalytics(startDate, endDate, undefined, adminContext);
            expect(analytics).toHaveLength(1);
        });
    });
    describe("getUserAnalyticsSummary", () => {
        it("should return user summary for own data", async () => {
            const userId = "user-123";
            const userContext = { userId, isAdmin: false };
            const summary = await analyticsDAO.getUserAnalyticsSummary(userId, userContext);
        ...
        it("should allow admin to view any user summary", async () => {
            const userId = "user-123";
            const adminContext = { userId: "admin-456", isAdmin: true };
            const summary = await analyticsDAO.getUserAnalyticsSummary(userId, adminContext);
            expect(summary.totalSessions).toBe(10);
            expect(summary.completedSessions).toBe(8);
        });
        it("should throw error when user tries to view other user data", async () => {
            ...
            await expect(analyticsDAO.getUserAnalyticsSummary(userId, otherUserContext)).rejects.toThrow("Insufficient privileges to view these analytics");
        });
    });
    describe("updateDailyAnalytics", () => {
        it("should create new analytics record", async () => {
            ...
            const analytics = await analyticsDAO.updateDailyAnalytics(userId, date, updates);
        ...
        it("should create system-wide analytics record", async () => {
            ...
            const analytics = await analyticsDAO.updateDailyAnalytics(null, date, updates);
    ...
    describe("getPlatformMetrics", () => {
        ...
        it("should throw error for non-admin user", async () => {
            const userContext = { userId: "user-123", isAdmin: false };
            await expect(analyticsDAO.getPlatformMetrics(userContext)).rejects.toThrow("Admin privileges required for platform metrics");
        });
    });
    describe("getAnalyticsRange", () => {
        it("should get analytics range for user", async () => {
            ...
            const userId = "user-123";
            const userContext = { userId, isAdmin: false };
            const analytics = await analyticsDAO.getAnalyticsRange(startDate, endDate, userId, userContext);
            expect(analytics).toHaveLength(1);
        });
        it("should allow admin to view any user analytics", async () => {
            ...
            const userId = "user-123";
            const adminContext = { userId: "admin-456", isAdmin: true };
            const analytics = await analyticsDAO.getAnalyticsRange(startDate, endDate, userId, adminContext);
            expect(analytics).toHaveLength(1);
        });
        it("should throw error when non-admin tries to view other user data", async () => {
            ...
            await expect(analyticsDAO.getAnalyticsRange(startDate, endDate, userId, otherUserContext)).rejects.toThrow("Admin privileges required to view other user analytics");
        });
    });
    describe("getSystemHealth", () => {
        ...
        it("should throw error for non-admin user", async () => {
            const userContext = { userId: "user-123", isAdmin: false };
            await expect(analyticsDAO.getSystemHealth(userContext)).rejects.toThrow("Admin privileges required for system health metrics");
        });
    });
...
//# sourceMappingURL=AnalyticsDAO.test.js.map