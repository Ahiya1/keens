...
describe('Logger System', () => {
    ...
    describe('Specialized Logging Methods', () => {
        ...
        test('should log tool execution success', async () => {
            ...
            await logger.logToolExecution('test_tool', { param1: 'value1' }, startTime, { result: 'success' });
            const stats = logger.getStats();
        ...
        test('should log tool execution failure', async () => {
            const startTime = Date.now();
            await logger.logToolExecution('test_tool', { param1: 'value1' }, startTime, undefined, new Error('Tool failed'));
            const stats = logger.getStats();
            expect(stats.totalLogs).toBe(1);
            expect(stats.toolExecutions).toBe(1);
