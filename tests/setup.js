...
const testDataRegistry = {
    ...
    sessions: new Set(),
    connections: new Set(),
    sessionIdMap: new Map(), // Maps test session IDs to UUIDs
...
console.error = (...args) => {
    ...
    if (typeof message === "string" &&
        (message.includes("Database connection test failed") ||
            ...
            message.includes('Database query error: error: relation "audit_logs" does not exist') ||
 ...
 */
export function getSessionUuid(sessionId) {
...
async function cleanupTestData() {
    ...
    try {
        ...
        if (userIds.length > 0) {
            await testDbManager.query(`DELETE FROM users WHERE id = ANY($1::uuid[])`, [userIds]);
 ...
 */
export async function createTestUser(email = null, username = null, isAdmin = false) {
 ...
 */
export async function createTestUserInDB(email = null, username = null, _password = "test123", // Use underscore prefix to indicate intentionally unused parameter
isAdmin = false) {
    if (!testDbManager) {
    ...
    try {
        ...
        const existingUsers = await testDbManager.query("SELECT id FROM users WHERE email = $1 OR username = $2", [testEmail, testUsername]);
        ...
        await testDbManager.query(`INSERT INTO users (id, email, username, password_hash, is_admin, admin_privileges, email_verified)
       VALUES ($1, $2, $3, $4, $5, $6, $7)`, [
        ...
        await testDbManager.query(`INSERT INTO credit_accounts (user_id, current_balance, unlimited_credits)
       VALUES ($1, $2, $3)`, [userId, isAdmin ? 0 : 100, isAdmin]);
        // Register for cleanup
...
export async function cleanupTestUsers(emails) {
    if (!testDbManager || emails.length === 0)
        return;
    try {
        const result = await testDbManager.query("DELETE FROM users WHERE email = ANY($1::text[])", [emails]);
 ...
 */
export function forceCleanupResources() {
