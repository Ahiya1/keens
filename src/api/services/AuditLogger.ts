/**
 * keen API Gateway - Audit Logging Service
...
export class AuditLogger {
  ...
  /**
   * Create audit log entry - FIXED to use correct column name
   */
  private async createAuditEntry(
    entry: Omit<AuditLogEntry, "id"> & { agent_session_id?: string }
  ): Promise<void> {
    const id = uuidv4();
    ...
    try {
      await this.db.query(
        ...
        INSERT INTO audit_logs (
          id, event_type, user_id, agent_session_id, request_id, timestamp,
