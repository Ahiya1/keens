...
export class GetAgentStatusTool {
  ...
  async execute(
    ...
    try {
      ...
      console.log(chalk.blue(`📈 Generating Agent Tree Status Report...`));
      ...
      if (sortedNodes.length > 0) {
        console.log(chalk.blue(`\n📄 Agent Details (${sortedNodes.length} agents):`));
      ...
      console.log(chalk.red(`❌ Failed to get agent status: ${error.message}`));
  ...
  private displayStatusSummary(
    ...
    console.log(chalk.gray(`Root Session: ${treeStatus.rootSessionId}`));
    console.log(chalk.gray(`Total Agents: ${treeStatus.totalNodes}`));
    console.log(chalk.gray(`Active: ${treeStatus.activeNodes} | Completed: ${treeStatus.completedNodes} | Failed: ${treeStatus.failedNodes}`));
    console.log(chalk.gray(`Max Depth: ${treeStatus.maxDepth} | Filtered Results: ${filteredCount}`));
    
    if (currentNode) {
      console.log(chalk.yellow(`\n🤖 Current Agent:`));
      console.log(chalk.gray(`  ID: ${currentNode.sessionId}`));
      ...
      console.log(chalk.gray(`  Depth: ${currentNode.depth} | Children: ${currentNode.children.length}`));
      console.log(chalk.gray(`  Git Branch: ${currentNode.gitBranch}`));
    }
  }
  ...
  private displayMetrics(metrics: any): void {
    ...
    console.log(chalk.gray(`    Success Rate: ${(metrics.execution.successRate * 100).toFixed(1)}%`));
    console.log(chalk.gray(`    Active: ${metrics.execution.activeNodes}`));
    console.log(chalk.gray(`    Completed: ${metrics.execution.completedNodes}`));
    console.log(chalk.gray(`    Failed: ${metrics.execution.failedNodes}`));
    
    console.log(chalk.blue(`  Timing:`));
    console.log(chalk.gray(`    Average Duration: ${Math.round(metrics.timing.averageDuration / 1000)}s`));
    console.log(chalk.gray(`    Total Execution Time: ${Math.round(metrics.timing.totalExecutionTime / 1000)}s`));
    
    console.log(chalk.blue(`  Hierarchy:`));
    console.log(chalk.gray(`    Max Depth: ${metrics.hierarchy.maxDepth}`));
    console.log(chalk.gray(`    Average Depth: ${metrics.hierarchy.averageDepth.toFixed(1)}`));
    
    console.log(chalk.blue(`  Specializations:`));
    Object.entries(metrics.specialization.distribution).forEach(([spec, count]: [string, any]) => {
      console.log(chalk.gray(`    ${spec}: ${count}`));
    });
  }
